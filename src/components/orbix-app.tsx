...
function Field({ label, children }) {
  const T = useT();
  return <div><label style={{ fontSize:11,fontWeight:600,color:T.textSub,display:"block",marginBottom:5,letterSpacing:0.3 }}>{label}</label>{children}</div>;
}
  ...
  return (
    <Modal onClose={onClose} width={440}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
        <Field label="Category Name"><input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Legal, Operations…" style={{ ...IS,borderColor:name&&!valid?"#EF4444":T.border }}/>{name&&!valid&&<p style={{ margin:"4px 0 0",fontSize:11,color:"#EF4444" }}>Already exists.</p>}</Field>
  ...
  return (
    <Modal onClose={onClose} width={520}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:13 }}>
        {isTask?<>
          <Field label="Task Title"><input value={vals.title} onChange={e=>set("title",e.target.value)} placeholder="e.g. Review Q3 report" style={IS}/></Field>
          <Field label="Project"><select value={vals.project} onChange={e=>set("project",e.target.value)} style={IS}>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
          ...
          <Field label="Project Name"><input value={vals.name} onChange={e=>set("name",e.target.value)} placeholder="e.g. Q3 Campaign" style={IS}/></Field>
          <Field label="Department"><select value={vals.category} onChange={e=>set("category",e.target.value)} style={IS}>{depts.map(d=><option key={d.key} value={d.key}>{d.icon} {d.key}</option>)}</select></Field>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
            <Field label="Status"><select value={vals.status} onChange={e=>set("status",e.target.value)} style={IS}>{STATUSES.map(s=><option key={s}>{s}</option>)}</select></Field>
            <Field label="Start Date"><input type="date" value={vals.startDate} onChange={e=>set("startDate",e.target.value)} style={IS}/></Field>
            <Field label="Deadline"><input type="date" value={vals.deadline} onChange={e=>set("deadline",e.target.value)} style={IS}/></Field>
...
function AddMemberModal({ onSave, onClose, existingEmails }) {
  ...
  return (
    <Modal onClose={onClose} width={440}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
        <Field label="Full Name"><input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Jordan Smith" style={IS}/></Field>
        <Field label="Email"><input value={email} onChange={e=>setEmail(e.target.value)} placeholder="e.g. jordan@orbix.io" style={{ ...IS,borderColor:emailTaken?"#EF4444":T.border }}/>{emailTaken&&<p style={{ margin:"4px 0 0",fontSize:11,color:"#EF4444" }}>Email already exists.</p>}</Field>
...
function EditDeptModal({ dept, existingDepts, onSave, onClose }) {
  ...
  return (
    <Modal onClose={onClose} width={440}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
        <Field label="Department Name"><input value={name} onChange={e=>setName(e.target.value)} style={{ ...IS,borderColor:name&&!valid?"#EF4444":T.border }}/>{name&&!valid&&<p style={{ margin:"4px 0 0",fontSize:11,color:"#EF4444" }}>Name already exists.</p>}</Field>
        <Field label="Icon"><div style={{ display:"flex",flexWrap:"wrap",gap:7,paddingTop:4 }}>{DEPT_ICONS.map(ic=><div key={ic} onClick={()=>setIcon(ic)} style={{ width:36,height:36,borderRadius:10,background:ic===icon?color+"22":T.overlay,border:"2px solid "+(ic===icon?color:T.border),display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,cursor:"pointer" }}>{ic}</div>)}</div></Field>
        <Field label="Color"><div style={{ display:"flex",gap:7,flexWrap:"wrap",paddingTop:4 }}>{DEPT_COLORS.map(c=><div key={c} onClick={()=>setColor(c)} style={{ width:28,height:28,borderRadius:8,background:c,cursor:"pointer",border:color===c?"3px solid "+T.text:"3px solid transparent",boxSizing:"border-box" }}/>)}</div></Field>
